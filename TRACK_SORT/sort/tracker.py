...
class Tracker:
    ...
    def update(self, detections, confidences, kpts, line_kpts):
        ...
        for track_idx, detection_idx in matches:
            self.tracks[track_idx].update(
                detections[detection_idx], confidences[detection_idx], kpts[detection_idx], line_kpts[detection_idx])
        for track_idx in unmatched_tracks:
            self.tracks[track_idx].mark_missed()
        for detection_idx in unmatched_detections:
            self._initiate_track(detections[detection_idx], confidences[detection_idx].item(),
                                 kpts[detection_idx], line_kpts[detection_idx])
    ...
    def _initiate_track(self, detection, conf, kpt, line_kpt):
        self.tracks.append(Track(
            detection.to_xyah(), self._next_id, conf, kpt, line_kpt, self.n_init, self.max_age))
        self._next_id += 1
