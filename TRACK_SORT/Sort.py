...
class SORT(object):
    ...
    def update(self, bbox_xywh, confidences, kpts, line_pt, img_org):
        self.height, self.width = img_org.shape[:2]
        bbox_xywh = self._xyxy_to_xywh(bbox_xywh)
        bbox_tlwh = self._xywh_to_tlwh(bbox_xywh)
        detections = [Detection(bbox_tlwh[i], conf, kpts[i], line_pt[i]) for i, conf in enumerate(
            confidences)]
        ...
        line_keypoints = np.array(d.line_kpt for d in detections)

        # update tracker
        self.tracker.predict()
        self.tracker.update(detections, confidences, kpts, line_pt)
        ...
        for track in self.tracker.tracks:
            ...
            conf = track.conf
            kpt = track.kpt
            line_kpt = track.line_kpt
            pts = np.array(track.keypoints_list, dtype=np.float32)
            outputs.append({'bbox': [x1, y1, x2, y2], 'score': conf, 'kpt': kpt, 'line_kpt': line_kpt, 'id': track_id, 'list_kpt': pts})
        return outputs
